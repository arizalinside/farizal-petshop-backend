...
const Sales = {
  getAllSales: async () => {
    const [rows] = await db.query(
      "SELECT * FROM sales WHERE deleted_at IS NULL ORDER BY updated_at DESC, id DESC"
    );
    return rows;
  },
  ...
  getSalesWithPagination: async (limit, offset) => {
    const [rows] = await db.query(
      "SELECT * FROM sales WHERE deleted_at IS NULL ORDER BY updated_at DESC, id DESC LIMIT ? OFFSET ?",
      [limit, offset]
    );
